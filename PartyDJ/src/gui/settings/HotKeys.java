...
public class HotKeys extends javax.swing.JPanel 
{
	...
	protected void initFuntions()
	{
		...
		clearGlobal.addActionListener(new ActionListener()
		{
			...
			{
				for(final String cmd : KeyStrokeManager.getInstance().getGlobalHotKeys())
				{
	...
	protected static void setGlobalFieldText(final JTextField...fields)
	{
		final IData data = Controller.getInstance().getData();
		final JTextField[] txtFields = fields;
		...
		if(raw != null)
		{
			if(raw.length() < 1)
				return;
			raw = raw.substring(1);
			final String[] regKeys = raw.split("§");
			for(final String k : regKeys)
			{
				final String[] key = k.split("@");
				for(final JTextField toSet : txtFields)
				{
					if(key[2].equalsIgnoreCase(toSet.getName()))
					{
						final String[] represent = KeyStroke.getKeyStroke(Integer.valueOf(key[1]), Integer.valueOf(key[0])).toString().split("pressed ");
	...
	protected static void setLocalFieldText(final JTextField...fields)
	{
		final IData data = Controller.getInstance().getData();
		final JTextField[] txtFields = fields;
	...
	static protected class GlobalHotKeySetter implements KeyListener
	{
		...
		public GlobalHotKeySetter(final JTextField...txts)
		{
			fields = txts;
		}
		
		@Override
		public void keyPressed(final KeyEvent e) { /* not to implement */ }
		...
		@Override
		public void keyTyped(final KeyEvent e) { /* not to implement */ }
	}
	
	static protected class LocalHotKeySetter implements KeyListener
	{
		protected JTextField[] fields;
		protected KeyStrokeManager manager = KeyStrokeManager.getInstance();
		protected int key = 0;
		...
		public LocalHotKeySetter(final JTextField...txts)
		{
			fields = txts;
		}
		...
		public void keyReleased(final KeyEvent e)
		{
			final InputMap iMap = manager.getInputMap();
